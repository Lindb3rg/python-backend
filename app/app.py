...
def create_product(create_product: ProductCreate,
    ...
    logger.info(
        ...
        extra={
            ...
            "product_name": create_product.name,
    ...
    try:
        ...
        logger.success(f"Product '{product.name}' created successfully", extra={"product_id": product.id})
        return product
...
def read_products(
    ...
    logger.info(f"Retrieved {len(products)} products", extra={
        "count": len(products),
        "offset": offset,
        "limit": limit
    })
...
def delete_product(product_id: int, session: SessionDep):
    ...
    try:
        ...
        session.commit()
        logger.success(f"Product '{product_name}' deleted successfully", extra={"product_id": product_id})
        return {"ok": True}
...
def update_product(product_id: int, product: ProductUpdate, session: SessionDep):
    ...
    if not product_data:
        logger.warning(f"No data provided for product update", extra={"product_id": product_id})
    ...
    try:
        ...
        logger.success(f"Updated product successfully", extra={"product_id": product_db.id})
        return product_db
...
def read_orders(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(orders)} orders", extra={
            "count": len(orders),
            "offset": offset,
            "limit": limit
        })
...
def delete_order(order_id: int, session: SessionDep):
    ...
    try:
        ...
        logger.success(f"Order {order_id} and {len(order_details)} details deleted successfully")
        return {"ok": True}
...
def update_order(order_id: int, order: OrderUpdate, session: SessionDep):
    ...
    if not order_data:
        logger.warning(f"No data provided for order update", extra={"order_id": order_id})
        raise HTTPException(status_code=422, detail="Unprocessable Entity")
    ...
    try:
        ...
        logger.success(f"Order {order_id} updated successfully", extra={"order_id": order_id})
        return order_db
...
def create_order_batch(orders_data: OrderBatchCreate, session: SessionDep):
    ...
    order_batch = OrderBatch(authentication_string=orders_data.authentication_string)
    ...
    try:
        for order_idx, order in enumerate(orders_data.order_list):
            logger.info(f"Processing order {order_idx + 1}/{len(orders_data.order_list)} for {order.customer_email}")
            ...
            for item in order.items:
                ...
                if not product:
                    logger.error(f"Product {item.product_id} not found in order {order_idx + 1}")
                    raise HTTPException(
                        status_code=404,
                ...
                if product.stock_quantity < item.quantity:
                    logger.error(f"Insufficient stock for {product.name}: {product.stock_quantity} < {item.quantity}")
                    raise HTTPException(
                        status_code=400,
            ...
            logger.info(f"Order created for {order.customer_email} with total ${total_amount}")
        ...
        logger.success(f"Order batch created successfully with {len(created_orders)} orders")
        return order_batch
